...
local string_split = _G.string_split
...
function SC_SetupOptionsUI()
	...
	SC_AutoGrats.ui.guildCheckButton:SetScript("OnShow", function(self)
		self:SetChecked(SC_Guild)
	end)

	SC_AutoGrats.ui.guildCheckButton:SetScript("OnClick", function()
		SC_ToggleGuild()
	end)
	...
	SC_AutoGrats.ui.partyCheckButton:SetScript("OnShow", function(self)
		self:SetChecked(SC_Party)
	end)

	SC_AutoGrats.ui.partyCheckButton:SetScript("OnClick", function()
		SC_ToggleParty()
	end)
	...
	SC_AutoGrats.ui.sayCheckButton:SetScript("OnShow", function(self)
		self:SetChecked(SC_Say)
	end)

	SC_AutoGrats.ui.sayCheckButton:SetScript("OnClick", function()
		SC_ToggleSay()
	end)
	...
	SC_AutoGrats.ui.guildWelcomeCheckButton:SetScript("OnShow", function(self)
		self:SetChecked(SC_GuildJoinMessageToggle)
	end)

	SC_AutoGrats.ui.guildWelcomeCheckButton:SetScript("OnClick", function()
	...
	SC_AutoGrats.ui.delaySlider:HookScript("OnValueChanged", function(_, value)
		SC_Delay = math_floor(value)
	end)

	SC_AutoGrats.ui.delaySlider:HookScript("OnShow", function(self)
		self:SetValue(SC_Delay)
		self.editbox:SetNumber(SC_Delay)
	end)

	SC_AutoGrats.ui.delaySlider.editbox:SetScript("OnShow", function(self)
	...
	SC_AutoGrats.ui.groupingSlider:HookScript("OnValueChanged", function(_, value)
		SC_GroupingTime = math_floor(value)
	end)

	SC_AutoGrats.ui.groupingSlider:HookScript("OnShow", function(self)
		self:SetValue(SC_GroupingTime)
		self.editbox:SetNumber(SC_GroupingTime)
	end)

	SC_AutoGrats.ui.groupingSlider.editbox:SetScript("OnShow", function(self)
	...
	SC_AutoGrats.ui.gratsMessageEditBox:SetScript("OnShow", function(self)
		self:SetText(SC_GratsMessage1)
		self:SetCursorPosition(0)
		self:ClearFocus()
	end)

	SC_AutoGrats.ui.gratsMessageEditBox:SetScript("OnTextChanged", function(self)
	...
	SC_AutoGrats.ui.guildWelcomeMessageEditBox:SetScript("OnShow", function(self)
		self:SetText(SC_GuildWelcomeMessage1)
		self:SetCursorPosition(0)
		self:ClearFocus()
	end)

	SC_AutoGrats.ui.guildWelcomeMessageEditBox:SetScript("OnTextChanged", function(self)
...
function SC_OnEvent(self, event, arg1, arg2)
	...
	if (not SC_IsMe(arg2)) then
		if (event == "CHAT_MSG_GUILD_ACHIEVEMENT" and not SC_GuildDisabledOverride) then
			SC_DoGrats("GUILD")
...
function SC_DoGrats(source)
	if ((source == "SAY" and SC_Say == true) or (source == "GUILD" and SC_Guild == true) or (source == "PARTY" and SC_Party == true)) then
		...
		if ((CurTime - SC_LastMessage) > (SC_GroupingTime/1000)) then
			local argsGrats = {SC_GratsMessage1, SC_GratsMessage2, SC_GratsMessage3}
			SC_LastMessage = GetTime()
			if (SC_Delay > 0) then
				C_Timer_After((SC_Delay / 1000), function()
					SendChatMessage(argsGrats[math_random(#argsGrats)], source)
				end)
			else
				SendChatMessage(argsGrats[math_random(#argsGrats)], source)
			end
...
function SC_GuildWelcome()
	print(SC_GuildWelcomeMessage1)
	if (SC_GuildJoinMessageToggle and (GetTime() - SC_LastMessage > (SC_GroupingTime/1000))) then
		local argsWelcome = {SC_GuildWelcomeMessage1, SC_GuildWelcomeMessage2, SC_GuildWelcomeMessage3}
		SC_LastMessage = GetTime()
		if (SC_Delay > 0) then
			C_Timer_After((SC_Delay / 1000), function()
				SendChatMessage(argsWelcome[math_random(#argsWelcome)], "GUILD")
			end)
		else
			SendChatMessage(argsWelcome[math_random(#argsWelcome)], "GUILD")
		end
...
function CreateBasicSlider(parent, name, title, minVal, maxVal)
	...
	slider.textLow:SetText(math_floor(minVal))
	slider.textHigh:SetText(math_floor(maxVal))
	...
	slider:SetScript("OnValueChanged", function(self, value)
		self.editbox:SetNumber(math_floor(value))
	...
	editbox:SetScript("OnTextChanged", function(self)
		...
		if tonumber(value) then
			if (math_floor(value) > maxVal) then
				self:SetNumber(maxVal)
			end

			if math_floor(self:GetParent():GetValue()) ~= math_floor(value) then
				self:GetParent():SetValue(math_floor(value))
			end
		end
	end)
